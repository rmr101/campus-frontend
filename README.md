...
1. 先创docker Dockerfile
  ？ workDIR 
2. Build image
	
	
3. docker run -it -p 3000:3000 campus-frontend
...
4. docker compose 
   1. volumes (马上映射)
   2. $ docker-compose up 
      1. stdin_open:true 要加
可以用docker compose 跑 CICD 也可以不这么干。

5. Build image from Docker file production
	默认端口号是80
	build again then run
	把compose 改成80 对80

6. 用travis CI 来管理github 
